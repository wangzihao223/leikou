...
-spec minimum_jumps(Forbidden :: [integer()],
                    A :: integer(),
                    B :: integer(),
                    X :: integer()) ->
                       integer().
minimum_jumps(_, _, _, 0) ->
    0;
minimum_jumps(Forbidden, A, B, X) ->
    jump(queue:from_list([{0, -1, 0}]),
         sets:from_list(Forbidden, [{version, 2}]),
         sets:add_element({0, 1}, sets:new([{version, 2}])),
         max(lists:max(Forbidden) + A + B, X + B),
         {A, B},
         X).
...
jump(Q, Forbidden, Trodden, Max, AB, X) ->
    case queue:is_empty(Q) of
        true ->
            -1;
        false ->
            jump1(Q, Forbidden, Trodden, Max, AB, X)
    end.
...
operation([{D, Dis} | Next], Q, Now, Forbidden, Trodden, Max, X, Res, AB) ->
    case walk(Q, Now, Forbidden, D, Dis, Trodden, Max, X, Res) of
        {ok, Res1} ->
            Res1;
        {Trodden1, Q1} ->
            operation(Next, Q1, Now, Forbidden, Trodden1, Max, X, Res, AB)
    end;
...
walk(Q, Now, Forbidden, D, Dis, Trodden, Max, X, Res) ->
    ...
    if E == X ->
           {ok, Res + 1};
       true ->
           case not sets:is_element(E, Forbidden)
                and not sets:is_element({E, Dis}, Trodden)
                and ((E >= 0) and (E =< Max))
           of
               true ->
                   {sets:add_element({E, Dis}, Trodden), queue:in({E, Dis, Res + 1}, Q)};
               false ->
                   {sets:add_element({E, Dis}, Trodden), Q}
           end
    end.
