...
init_window(_Window, _WinMap, [], Length, _Map, _Index, _SourceLength) when Length > 0 -> false;
init_window(Window, WinMap, [L | NextL], Length, Map, Index, SourceLength) when Length > 0->
  case maps:is_key(L, Map) of
    true ->
      init_window(queue:in([L, Index], Window), WinMap#{L => maps:get(L, WinMap, 0) + 1}, NextL, Length - 1, Map, Index + 1,  SourceLength);
    false ->
      init_window(queue:new(), #{}, NextL, SourceLength, Map, Index + 1, SourceLength)
  end;
init_window(Window, WinMap, Remain, _, _, Index, _SourceLength) -> {Window, WinMap, Remain, Index}.
