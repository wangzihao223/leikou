...
init_window([N | NextN], Window, Heap, Index, Length) when Length > 0 ->
  init_window(NextN, queue:in({N, Index}, Window), insert(N, Index, Heap), Index + 1, Length - 1);
init_window(Nums, Window, Heap, Index, _) -> {Nums, Window, Heap, Index}.
...
move_window([], _, _, _, Res, _) -> lists:reverse(Res).
