...
loop_col([], _, _, RowsMap, ColsMap) -> {RowsMap, ColsMap}.
...
loop_row([], _, RowsMap, ColsMap) -> {RowsMap, ColsMap}.


loop_col_1([C | Next], I, J, RMap, CMap, Count) when C == 1 ->
    Count1 = case (maps:get(I, RMap, 0) > 1) or (maps:get(J, CMap, 0) > 1) of
        true -> Count + 1;
        false -> Count
    end,
...
loop_col_1([], _, _, _, _, Count) -> Count.
